...
def _calculer(self, cr, uid, data, context):
    pool = pooler.get_pool(cr.dbname)
    pooler.get_pool(cr.dbname).get('asset.simulation').calculer(cr,uid,data['ids'])
    return{}


class wizard_calculer(wizard.interface):
    states = {
        'init': {
            'actions': [_init],
            #'result': {'type':'form', 'arch':calculer_form, 'fields':calculer_fields, 'state':[('end','Cancel'),('calculer','Calculer')]}
            'result': {'type':'state',  'state':'calculer'}    
        },
        'calculer': {
            'actions': [_calculer],

...
def _amortir(self, cr, uid, data, context):
	...
	ligne = pool.get('account.invoice.line').read(cr,uid,data['ids'][0]) # lit les valeur de la ligne de facture
	data=data['form']
	...
	print asset_id
	#pool.get('account.invoice.line').write(cr,uid,ligne[1],{'asset_id': asset_id}, context)
		
	return{}

class wizard_amortir_line(wizard.interface):
	states = {
		'init': {
			'actions': [_init],
			'result': {'type':'form', 'arch':amort_form, 'fields':amort_fields, 'state':[('end','Cancel'),('amortir','Amortir')]}
		},
		'amortir': {
			'actions': [_amortir],
...
wizard_amortir_line('amortir.line')

...
class resultat(report_sxw.rml_parse):
	def __init__(self, cr, uid, name, context):
		super(resultat, self).__init__(cr, uid, name, context)
	...
	def chargement(self,form):
			requete = "select name,definition,exceptions,valeur,type,somme from report_account_fr_parametres where somme = 'f' and report = 'resultat'"
			self.cr.execute(requete)
			resultats =self.cr.dictfetchall()
			for resultat in resultats:
				self.donnees(form,eval(resultat['definition']),resultat['name'],eval(resultat['exceptions']),resultat['type'],resultat['valeur'])
			requete = "select name,definition,exceptions,valeur,type,somme from report_account_fr_parametres where somme = 't' and report = 'resultat'"
			self.cr.execute(requete)
			resultats =self.cr.dictfetchall()
			#print "requete somme",resultats
			for resultat in resultats:
				variable=resultat['name']
				res =self.somme(eval(resultat['definition']))
...
report_sxw.report_sxw('report.report.account.fr.resultat', 'report.account.fr.lignes','addons/report_account_fr/report/compte_de_resultat.rml', parser=resultat, header=False)
